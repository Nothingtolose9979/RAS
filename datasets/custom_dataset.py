...
def build_custom_dataloader(cfg, training, distributed=True):
    ...
    if training:
        ...
        print("Number of validation/test data: ", len(dataset))
    ...
    def __init__(
        ...
        with open(meta_file, "r") as f_r:
            ...
            for line in f_r:
                ...
                if self.clsname is None or self.clsname == "all":
                    self.metas.append(meta)
                else:
                    if meta.get("clsname", None):
                        clsname = meta["clsname"]
                    else:
                        clsname = filename.split("/")[-4]

                    if clsname == self.clsname:
                        self.metas.append(meta)
        print("Number of metas: ", len(self.metas))
        assert len(self.metas) > 0, "no metas. check clsname: " + self.clsname
