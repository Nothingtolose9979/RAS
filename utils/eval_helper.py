...
def performance_ensemble(fileinfos_list, preds_list, masks_list, config):
    ret_metrics = {}
    clsnames = set([fileinfo["clsname"] for fileinfo in fileinfos_list[0]])
    for clsname in clsnames:
        preds_cls = []
        masks_cls = []
        # for fileinfo, pred, mask in zip(fileinfos, preds, masks):
        for i in range(len(fileinfos_list[0])):
            fileinfo = fileinfos_list[0][i]
            assert all([finfo[i]["clsname"] == fileinfo["clsname"]  for finfo in fileinfos_list])
            if fileinfo["clsname"] == clsname:
                pred_tmp = sum([p[i][None, ...] for p in preds_list]) / len(preds_list)
                preds_cls.append(pred_tmp)

                mask_tmp = sum([m[i][None, ...] for m in masks_list]) / len(masks_list)
                masks_cls.append(mask_tmp)
...
def log_metrics(ret_metrics, config, is_best=False):
    logger = logging.getLogger("global_logger")
    if is_best:
        logger.info("********The Best Performance**********")
